...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* Layout: Navigation Rail on Desktop & Tablets, Bottom Bar on Phones */
    .app-scaffold {
      display: flex;
      width: 100vw;
      min-height: 100vh;
    ...
    /* Material 3 Outlined Input Fields */
    .m3-field-wrap {
      position: relative;
      margin-bottom: 16px;
    }
    ...
    /* Camera Scanner Modal (For Phone) */
    #scanner-modal {
      display: none;
      position: fixed;
      inset: 0;
    ...
    /* Mobile Responsive Adaptations (< 600px) */
    @media (max-width: 600px) {
      ...
      /* On mobile show barcode camera button */
      .phone-only-btn {
    ...
    @media (min-width: 601px) {
      /* Hide phone barcode scanner button on desktop as requested */
      .phone-only-btn {
...
<div class="app-scaffold">
  <!-- Material 3 Navigation Rail (Desktop) / Navigation Bar (Phone) -->
  <nav class="nav-rail">
  ...
  <main class="main-content">
    ...
    <!-- 2. INVENTORY & ITEMS MANAGEMENT TAB -->
    <section id="tab-inventory" class="view-panel">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
        ...
        <div style="display: flex; gap: 10px;">
          ...
          <button class="m3-btn m3-btn-outlined" onclick="openSeedModal()">
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 16px;" id="inventory-pagination">
...
<!-- Camera Barcode Scanner Modal (Phone Exclusive) -->
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Auto debounce search suggestions
  ...
  async function lookupBarcodeFromInput() {
    ...
    try {
      ...
      // If not exact barcode, query search items
    ...
    searchDebounceTimer = setTimeout(async () => {
      try {
        ...
        container.innerHTML = data.items.map(it => `
          <div class="suggestion-item" onclick='selectSuggestedItem(${JSON.stringify(it).replace(/'/g, "&apos;")})'>
            <div>
              <div style="font-weight: 600;">\${it.name}</div>
              <div style="font-size: 11px; color: var(--md-sys-color-outline);">\${it.barcode} | \${it.brand || 'No Brand'}</div>
            </div>
      ...
      return `
        <tr>
          <td>
            ...
            <div style="font-size: 11px; color: var(--md-sys-color-outline);">
          ...
          <td>
            <input type="number" step="any" min="0.01" class="qty-input" value="\${item.qty}"
              onchange="updateCartItem(\${idx}, 'qty', this.value)">
          </td>
          <td>
            <input type="number" step="any" min="0" class="disc-input" value="\${item.trade_disc || ''}" placeholder="none"
              onchange="updateCartItem(\${idx}, 'trade_disc', this.value)" title="Divides (MRP x Qty) by this value">
          </td>
          <td>
            <input type="number" step="any" min="0" max="100" class="disc-input" value="\${item.disc || ''}" placeholder="0%"
              onchange="updateCartItem(\${idx}, 'disc', this.value)" title="Percent deducted from value">
          </td>
          <td style="font-weight: 700; color: var(--md-sys-color-primary);">
            ₹\${lineTotal.toFixed(2)}
          </td>
          <td>
            <button class="m3-btn m3-btn-danger m3-btn-sm" onclick="removeCartItem(\${idx})">
  ...
  function showReceiptModal(bill, items) {
    ...
    content.innerHTML = `
      <div style="text-align: center; border-bottom: 1px dashed #000; padding-bottom: 12px; margin-bottom: 12px;">
        ...
        <div style="font-size: 12px; margin-top: 4px;">Invoice: <b>\${bill.bill_number}</b></div>
        ...
        \${items.map(it => \`
          <div>
            ...
            <div style="display: flex; justify-content: space-between; font-size: 11px; color: #333;">
              <span>\${it.qty} x ₹\${it.mrp.toFixed(2)} \${it.trade_disc ? \`[÷\${it.trade_disc}]\` : ''} \${it.disc ? \`[-\${it.disc}%]\` : ''}</span>
      ...
      <div style="text-align: center; font-size: 11px; margin-bottom: 16px;">
  ...
  async function loadInventory(page = 1) {
    ...
    try {
      ...
      if (!data.items || data.items.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 20px;">No items found. Click "Seed Catalog" or "New Item".</td></tr>';
        return;
      }
      ...
      tbody.innerHTML = data.items.map(it => \`
        <tr>
          ...
          <td>\${it.brand || '<span style="color:var(--md-sys-color-outline)">-</span>'}</td>
          <td style="font-weight: 600;">₹\${Number(it.mrp).toFixed(2)}</td>
          <td>\${it.unit}</td>
          <td>\${it.stock}</td>
          <td>
            <div style="display: flex; gap: 6px;">
              <button class="m3-btn m3-btn-tonal m3-btn-sm" onclick='editItem(\${JSON.stringify(it).replace(/'/g, "&apos;")})'>
                <span class="material-symbols-outlined" style="font-size: 15px;">edit</span>
              </button>
              <button class="m3-btn m3-btn-danger m3-btn-sm" onclick="deleteItem(\${it.id})">
  ...
  async function openSeedModal() {
    const countStr = prompt("How many items from items.json would you like to seed into Cloudflare D1? (e.g. 50 or 200)", "50");
    if (!countStr) return;
    const count = parseInt(countStr, 10) || 50;

    showToast('Reading items.json and seeding D1...');
    try {
      // In web app, we can fetch items.json if served or upload a file
      alert('To seed items, you can use the backend /api/items/seed endpoint or wrangler d1 execute.');
    } catch (e) {
      alert(e.message);
    }
  }
    ...
    try {
      ...
      tbody.innerHTML = data.bills.map(b => \`
        <tr>
          ...
          <td><span class="chip">\${b.payment_mode}</span></td>
          <td>\${b.total_qty}</td>
          <td style="font-weight: 700; color: var(--md-sys-color-primary);">₹\${Number(b.grand_total).toFixed(2)}</td>
          <td>
            <button class="m3-btn m3-btn-tonal m3-btn-sm" onclick="viewPastBill(\${b.id})">
